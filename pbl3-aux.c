...
typedef struct{
    char hora[200];
    char umidade[200];
    char temperatura[200];
    char luminosidade[200];
    char pressao[200];
}medicao;

medicao medicao_temp;
medicao historico[10]; // array do histórico
...
void concatenar_umidade(int umidade_v1, int umidade_v2)
{
    ...
    strcpy(medicao_temp.umidade,strcat(strcpy(umi_final, text_umidade), umi_valor));
...
void concatenar_temperatura(int temperatura)
{
    ...
    strcpy(medicao_temp.temperatura ,strcat(strcpy(temp_final, text_temp), temp_valor));
...
void concatenar_luminosidade(float luminosidade)
{
    ...
    strcpy(medicao_temp.luminosidade ,strcat(strcpy(lum_final, text_luminosidade), lum_valor));
...
void concatenar_pressao(float pressao)
{
    ...
    strcpy(medicao_temp.pressao ,strcat(strcpy(press_final, text_pressao), press_valor));
...
void sensor()
{
    ...
    if ((j >= 40) && (dht11_dat[4] == ((dht11_dat[0] + dht11_dat[1] + dht11_dat[2] + dht11_dat[3]) & 0xFF)))
    {
        ...
        strftime(hora, 40, "%H:%M:%S", time_info);
        ...
        result = fprintf(arq, "\n\n");
        fclose(arq);
        // PREPARANDO STRUCT GLOBAL
        char hora_concat[50];
        strcpy(hora_concat, "\n");
        strcat(hora_concat, hora);
        strcat(hora_concat, "\n\n");
        strcpy(medicao_temp.hora, hora_concat);
        ...
        // aqui ja tenho minha strcut da medição atual pronta
        //atualiza o histórico
        for(int i = 10; i > 0; i--){
            historico[i] = historico[i-1];
        }
        // poe a medição mais atualizada no topo do array
        historico[0] = medicao_temp;
        char mensagem[500];
        for(int i = 0; i < sizeof(historico); i++){
            strcat(mensagem,historico[i].hora);
            strcat(mensagem,historico[i].umidade);
            strcat(mensagem,"\n");
            strcat(mensagem,historico[i].temperatura);
            strcat(mensagem,"\n");
            strcat(mensagem,historico[i].luminosidade);
            strcat(mensagem,"\n");
            strcat(mensagem,historico[i].pressao);
            strcat(mensagem,"\n");          
        }
